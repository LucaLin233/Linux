...
- 推送前，生产宿主机仅执行差异检查及改动脚本的 `bash -n`；分析和测试由 CI 完成：PR 按完整差异及依赖映射选择测试；公共设施或未知依赖改动回退全量，main 推送执行全量。
...
3. 实施满足目标的最小改动。
4. 执行差异检查（`git diff --check` 并复核 diff），仅对改动的 Shell 脚本运行 `bash -n`；生产宿主机不运行全量 ShellCheck 或完整测试集。分析和测试交由 CI 按上述选择规则执行；其他本机重测试必须满足上述隔离与资源限制规则。
