...
工具在每轮测试前及 iperf3 运行过程中每 0.1 秒检查网卡计数，并按扫描上限预留 1–5 GB；
达到提前停止阈值时立即终止本工具启动的 iperf3 并恢复 qdisc。45/90 GB 是外层安全上限，
预留量用于覆盖计数轮询、进程终止调度和高速线路在途流量；同接口上的其他业务流量也计入统计。流量上限与扫描速率不是同一概念：没有丢包时会直接判定无 policer，
...
```text
CAKE、TBF、NETEM、TAPRIO、非本工具 HTB、混合 mq 叶子，以及根队列或 mq 子队列的自定义参数、class/filter
```
