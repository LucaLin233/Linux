...
interrupt_dir="$TEMP_DIR/verify-interrupt"
interrupt_pid_file="$TEMP_DIR/verify-interrupt.pid"
verify_impl() {
...
kill -0 "$wrapper_pid" 2>/dev/null && fail "interrupted verify did not exit promptly"
wait "$wrapper_pid" 2>/dev/null || true
if kill -0 "$child_pid" 2>/dev/null; then
    ps -o pid,ppid,pgid,stat,cmd -p "$wrapper_pid,$child_pid" >&2 || true
    cat "$interrupt_log" >&2
    fail "interrupted verify left child process running"
fi
[[ ! -e "$interrupt_dir" ]] || fail "interrupted verify left temporary directory"
printf 'PASS: interrupted verify cleans child process and temporary directory\n'
